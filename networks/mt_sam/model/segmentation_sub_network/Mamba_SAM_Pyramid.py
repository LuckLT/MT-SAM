...
from nnunetv2.networks.ma_sam.model.segmentation_sub_network.image_encoder_cross import ImageEncoderViT
from nnunetv2.networks.ma_sam.model.segmentation_sub_network.mask_decoder_prototype import MaskDecoder
...
from nnunetv2.networks.ma_sam.model.segmentation_sub_network.transformer_prototype import TwoWayTransformer
import os
...
#     def __init__(self, in_chs, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16):
#         super().__init__()
...
#                     ConvBNReLU(self.in_chs, dim, kernel_size=1),
...
#                     ))
#         self.mamba = Mamba(
#             d_model=dim*self.pool_len+self.in_chs,  # Model dimension d_model
...
#         return sequence
class MambaLayer(nn.Module):
    def __init__(self, in_chs_list, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16):
        super().__init__()
        self.adapt_convs = nn.ModuleList([
            ConvBNReLU(in_chs, dim, kernel_size=1) for in_chs in in_chs_list
        ])
        self.pool_scales = self.generate_arithmetic_sequence(1, last_feat_size, last_feat_size // 4)
        self.pool_len = len(self.pool_scales)

        # 更新池化层的卷积层，使其输入通道数为拼接后的通道数
        self.pool_layers = nn.ModuleList([
            nn.Sequential(
                ConvBNReLU(dim * len(in_chs_list), dim, kernel_size=1),  # 输入通道数为拼接后的通道数
                nn.AdaptiveAvgPool2d(pool_scale)
            ) for pool_scale in self.pool_scales
        ])

        # 确保 Mamba 模块的输入通道数正确
        # 拼接后的通道数为 dim * (len(in_chs_list) + self.pool_len)
        self.mamba = Mamba(
            d_model=dim * (len(in_chs_list) + self.pool_len),  # 每个特征图经过池化后拼接
            d_state=d_state,
            d_conv=d_conv,
            expand=expand
        )
    ...
    def forward(self, skip_list):
        adapted_features = []
        target_size = (224, 224)  # 目标尺寸为 224x224

        for i, x in enumerate(skip_list):
            x = self.adapt_convs[i](x)  # 调整通道数
            x = F.interpolate(x, size=target_size, mode='bilinear', align_corners=False)  # 上采样到目标尺寸
            # print('x.shape', x.shape)
            adapted_features.append(x)

        # 拼接所有特征图
        x = torch.cat(adapted_features, dim=1)  # (B, C_total, H, W)
        # print('x_cat.shape', x.shape)
        # 处理池化特征
        ppm_out = [x]
        for pool_layer in self.pool_layers:
            pool_out = pool_layer(x)
            pool_out = F.interpolate(pool_out, size=target_size, mode='bilinear', align_corners=False)
            ppm_out.append(pool_out)

        x = torch.cat(ppm_out, dim=1)  # 拼接池化后的特征
        B, C, H, W = x.shape

        # 确保输入到 Mamba 模块的形状为 (B, N, C)
        x = rearrange(x, 'b c h w -> b (h w) c')  # 展平为 (B, N, C)
        x = self.mamba(x)  # 输入到 Mamba 模块
        x = rearrange(x, 'b (h w) c -> b c h w', h=H, w=W)  # 恢复为特征图形状
        return x
...
class ChannelCompression(nn.Module):
    def __init__(self, in_channels=1024, out_channels=1):
        super().__init__()
        self.conv1 = ConvBNReLU(in_channels, 512, kernel_size=3)  # 1024 -> 512
        self.conv2 = ConvBNReLU(512, 256, kernel_size=3)  # 512 -> 256
        # self.conv3 = ConvBNReLU(256, 128, kernel_size=3)  # 256 -> 128
        # self.conv4 = ConvBNReLU(128, 64, kernel_size=3)  # 128 -> 64
        # self.conv5 = ConvBNReLU(64, out_channels, kernel_size=3)  # 64 -> out_channels
    ...
    def forward(self, x):
        ...
        # x = self.conv5(x)
        return x
class Mamba_SAM_Pyramid(nn.Module):
    def __init__(self, num_classes, num_atlas, encoder_embed_dim=768, encoder_depth=12, encoder_num_heads=12,
        ...
        # load pretrained model
        # modelpath = '/home/litaozhao/nnUNet2/nnUNet/nnunetv2/nnUNetFrame/DATASET/nnUNet_results/Dataset601_Loc_pro/nnUNetTrainer_MambaVision__nnUNetPlans__2d/fold_all/checkpoint_best_1.pth'
        modelpath = '/home/litaozhao/nnUNet2/nnUNet/nnunetv2/nnUNetFrame/DATASET/nnUNet_results/Dataset602_PICAI_Loc/nnUNetTrainer_MambaVision__nnUNetPlans__2d/fold_all/checkpoint_172_best.pth'
        state_dict1 = torch.load(modelpath, map_location='cuda')['network_weights']
        self.load_state_dict(state_dict1)

        image_embedding_size = image_size // vit_patch_size
        in_chs_list = [80, 160, 320, 640]  # 对应 skip_list 的通道数
        self.channelComp = ChannelCompression()
        # 初始化 MambaLayer
        self.mamba_layer = MambaLayer(in_chs_list=in_chs_list, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16)
    ...
    def freeze_layers(self, num_layers=100):
        ...
        for name, param in self.hmtunet.named_parameters():
            ...
            # else:
            #     break
    # def forward(self, img1, sim_map):
    #     with torch.no_grad():
    #         x, skip_list = self.hmtunet(img1)
    #         """Pyramid feature * similar map"""
    #         Pyramid_feature = self.mamba_layer(skip_list)  # [B, 1024, 224, 224]
    #     # print('Pyramid_feature.shape', Pyramid_feature.shape)
    #     Pyramid_feature = self.channelComp(Pyramid_feature)    # [B, 256, 224, 224]
    #     # sim_map[sim_map < 0] = 0
    #
    #     Pyramid_feature.permute(2, 3, 0, 1).flatten()
    #
    #     lesion_protype = Pyramid_feature * sim_map  # [B, 256, 224, 224] * [B, 1, 224, 224] = [B, 256, 224, 224]
    #     lesion_protype = lesion_protype.flatten(2)   # [B, 256, 224 * 224]
    #     lesion_protype = torch.mean(lesion_protype, dim=-1) # [B, 256]
    #     lesion_protype = lesion_protype.unsqueeze(-1).repeat(1, 1, 198)
    #     lesion_protype = lesion_protype.permute(0, 2, 1)
    #     # lesion_protype = lesion_protype / lesion_protype.norm(dim=-1, keepdim=True)
    #     # lesion_protype = torch.unsqueeze(lesion_protype, dim=1) # [B, 1, 224, 224]
    #
    #     '''
    #     Given: input_x
    #     template_fg = (template_lesion_region * (template_mask))
    #     template_bg = (template_lesion_region * (1 - template_mask))
    #     input_x_fg = SIM(template_fg, input_x) # globally lesion similarity
    #     input_x_bg = SIM(template_bg(no lesion), input_x) # globally non-lesion similarity
    #     out_sim = input_x_fg - input_x_bg
    #     out_sim[out_sim < 0] = 0
    #     '''
    #
    #     x = x[:, 1, :, :].unsqueeze(1)   # (B, 1, 224, 224)
    #     imgFeatures = self.image_encoder(img1, skip_list)  #imgFeatures.shape torch.Size([1, 256, 32, 32])
    #     promptFeatures, skip = self.prompt_encoder(x)
    #     promptFeatures_final = promptFeatures
    #     mask = self.mask_decoder(image_embeddings=imgFeatures,
    #                             pe=self.pe(),
    #                             prompt_embeddings=promptFeatures_final,
    #                             prompt_skip=skip,
    #                             prototype=lesion_protype,)
    #
    #     return mask
    ...
    def forward(self, img1, sim_map):
        ...
        # sim_dir = '/home/litaozhao/nnUNet2/nnUNet/nnunetv2/nnUNetFrame/DATASET/nnUNet_raw/Dataset604_Cross/similarity_map_Ts'
        # sim_path = os.path.join(sim_dir, name + '.nii.gz')
        # sim_img = sit
        # print('sim_map.shape',sim_map.shape)
        sim_map = sim_map[:, 0, :, :].unsqueeze(1)
        # print('sim_map2.shape', sim_map.shape)
        lesion_protype = Pyramid_feature * sim_map  # [B, 256, 224, 224] * [B, 1, 224, 224] = [B, 256, 224, 224]
        lesion_protype = lesion_protype.flatten(2)   # [B, 256, 224 * 224]
        lesion_protype = torch.mean(lesion_protype, dim=-1) # [B, 256]
        lesion_protype = lesion_protype.unsqueeze(-1).repeat(1, 1, 198)
...
# 定义特征图的通道数
# 假设 skip_list 是从模型中提取的特征图列表
# 定义特征图的通道数
# in_chs_list = [80, 160, 320, 640]  # 对应 skip_list 的通道数
#
# # 初始化 MambaLayer
# mamba_layer = MambaLayer(in_chs_list=in_chs_list, dim=128, d_state=16, d_conv=4, expand=2, last_feat_size=16).cuda()
#
# # 定义 skip_list
# skip_list = [
#     torch.randn(2, 80, 56, 56).cuda(),  # level 0
#     torch.randn(2, 160, 28, 28).cuda(),  # level 1
#     torch.randn(2, 320, 14, 14).cuda(),  # level 2
#     torch.randn(2, 640, 7, 7).cuda()  # level 3
# ]
#
# # 将整个 skip_list 输入到 MambaLayer
# Pyramid_feature = mamba_layer(skip_list)
# print(Pyramid_feature.shape)  # 输出形状