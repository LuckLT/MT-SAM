...
def _convert_to_npy(npz_file: str, unpack_segmentation: bool = True, overwrite_existing: bool = False,
    ...
    seg_npy2 = npz_file[:-4] + "_seg2.npy"
    # seg_npy3 = npz_file[:-4] + "_seg3.npy"
    try:
        ...
        if unpack_segmentation and (overwrite_existing or not isfile(seg_npy2)):
            try:
                npz_content = np.load(npz_file) if npz_content is None else npz_content
            except Exception as e:
                print(f"Unable to open preprocessed file {npz_file}. Rerun nnUNetv2_preprocess!")
        ...
        # if unpack_segmentation and (overwrite_existing or not isfile(seg_npy3)):
        #     try:
        #         npz_content = np.load(npz_file) if npz_content is None else npz_content
        #     except Exception as e:
        #         print(f"Unable to open preprocessed file {npz_file}. Rerun nnUNetv2_preprocess!")
        #         raise e
        #     np.save(npz_file[:-4] + "_seg3.npy", npz_content['seg3'])
        ...
        if verify_npy:
            try:
                ...
                if isfile(seg_npy2):
                    np.load(seg_npy2, mmap_mode='r')
                # if isfile(seg_npy3):
                #     np.load(seg_npy3, mmap_mode='r')
            except ValueError:
                os.remove(data_npy)
                os.remove(seg_npy)
                os.remove(seg_npy2)
