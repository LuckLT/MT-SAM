...
from nnunetv2.training.dataloading.utils_cross import get_case_identifiers


class nnUNetDataset_Cross(object):
    ...
    def load_case(self, key):
        ...
        # print('entry[data_file]:',entry['data_file'])
        # print('-------------------------------------------------')
        ...
        if 'open_seg_file' in entry.keys():
            ...
            seg2 = np.load(entry['data_file'][:-4] + "_seg2.npy", 'r')
            # seg3 = np.load(entry['data_file'][:-4] + "_seg3.npy", 'r')
            if self.keep_files_open:
            ...
            # seg3 = np.load(entry['data_file'])['seg3']
        if 'seg_from_prev_stage_file' in entry.keys():
            ...
            seg2 = np.vstack((seg2, seg_prev[None]))
            # seg3 = np.vstack((seg3, seg_prev[None]))

        # return data, seg, seg2, seg3, entry['properties']
...
if __name__ == '__main__':
    ...
    ds = nnUNetDataset_Cross(folder, num_images_properties_loading_threshold=0) # this should not load the properties!
    ...
    # this should have the properties
    ds = nnUNetDataset_Cross(folder, num_images_properties_loading_threshold=1000)
    ...
    ds = nnUNetDataset_Cross(folder, num_images_properties_loading_threshold=0)
    # now rename the properties file so that it does not exist anymore
    shutil.move(join(folder, 'liver_0.pkl'), join(folder, 'liver_XXX.pkl'))
