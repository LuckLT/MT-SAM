...
from nnunetv2.inference.predict_from_raw_data_MambaSAM_Pyramid import nnUNetPredictor
...
from nnunetv2.networks.ma_sam.model.segmentation_sub_network.Mamba_SAM_Pyramid import Mamba_SAM_Pyramid, HMTUNet
import argparse
import torch.nn.functional as F
import torch._dynamo
torch._dynamo.config.suppress_errors = True
class nnUNetTrainer_MambaSAM_Pyramid(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        # 添加这行来记录训练开始时间
        self._training_start_time = time()
        # 添加这行来记录每个epoch的时间
        self._epoch_times = []
    def initialize(self):
    ...
    def build_network(self):
        net = Mamba_SAM_Pyramid(num_classes=2, num_atlas=1, encoder_embed_dim=768, encoder_depth=12, encoder_num_heads=12,
    ...
    def on_train_start(self):
        ...
        # 在方法末尾添加
        self.print_model_parameters()  # 打印参数统计
    ...
    def on_train_end(self):
        ...
        # 在方法末尾添加
        if self.local_rank == 0:
            total_time = time() - self._training_start_time
            ...
            print("=" * 60)
            print(f"Total training time: {total_h}h {total_m}m {total_s}s ({total_time:.2f}s)")
            print(f"Total epochs: {self.num_epochs}")
            print(f"Average epoch time: {sum(self._epoch_times) / len(self._epoch_times):.2f}s")
            print("=" * 60 + "\n")
            ...
            time_stats = {
                ...
                'average_epoch_time_seconds': sum(self._epoch_times) / len(self._epoch_times),
                'epoch_times_seconds': self._epoch_times
            }
            with open(join(self.output_folder, 'time_stats.json'), 'w') as f:
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        target = batch['target']
        data_map_dif = batch['target2']
        # print('data_map_dif.shape', data_map_dif.shape)
        ...
        if isinstance(data_map_dif, list):
            data_map_dif = [i.to(self.device, non_blocking=True) for i in data_map_dif]
        else:
            data_map_dif = data_map_dif.to(self.device, non_blocking=True)

        self.optimizer.zero_grad(set_to_none=True)

        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            out = self.network(data, data_map_dif)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            output = self.network(data, data_map_dif)
            del data
    ...
    def on_epoch_end(self):
        ...
        # 计算当前epoch的时间
        epoch_start_time = self.logger.my_fantastic_logging['epoch_start_timestamps'][-1]
        epoch_end_time = self.logger.my_fantastic_logging['epoch_end_timestamps'][-1]
        ...
        self.print_to_log_file(
            f"Epoch {self.current_epoch + 1}/{self.num_epochs} time: {epoch_min}m {epoch_sec}s ({epoch_time:.2f}s)")
        self.print_to_log_file(f"Average epoch time: {avg_epoch_time:.2f}s")
        self.print_to_log_file(f"Estimated remaining: {remaining_h}h {remaining_m}m")
        self.print_to_log_file(f"Total training time so far: {total_h}h {total_m}m {total_s}s")
        self.print_to_log_file("-" * 50)
        self.print_to_log_file(
            f"Epoch time: {np.round(self.logger.my_fantastic_logging['epoch_end_timestamps'][-1] - self.logger.my_fantastic_logging['epoch_start_timestamps'][-1], decimals=2)} s")
        # 保存每个epoch的检查点
        if self.current_epoch >= 1:
            checkpoint_filename = join(self.output_folder, f'checkpoint_{self.current_epoch}.pth')
    ...
    def print_model_parameters(self):
        """打印模型参数统计"""
        if self.local_rank == 0:  # 只在主进程打印
            total_params = sum(p.numel() for p in self.network.parameters())
            trainable_params = sum(p.numel() for p in self.network.parameters() if p.requires_grad)
            frozen_params = total_params - trainable_params
            model_size_gb = total_params * 4 / 1024 ** 3  # float32 = 4 bytes
            ...
            for name, module in self.network.named_children():
                ...
                print(f"{name:30} : {module_params / 1e6:6.2f} M total, {module_trainable / 1e6:6.2f} M trainable")
            print("=" * 60 + "\n")
