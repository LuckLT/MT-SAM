...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, FocalLoss
...
from nnunetv2.networks.SwinUNet_class import SwinUnet_class
import nnunetv2.networks.SwinUnet_configs as get_config
import argparse
class nnUNetTrainer_SwinUNet_Class(nnUNetTrainer):
    ...
    def _build_loss(self):
        ...
        return loss
    def _build_loss_class(self):

        # loss_class = torch.nn.CrossEntropyLoss()
        # loss_class = torch.nn.BCEWithLogitsLoss()
        # class_counts = torch.tensor([3362, 397])
        # weight = 1.0 / class_counts.float()
        # weight /= weight.sum()
        # weight = weight.to('cuda')
        # loss_class = FocalLoss(gamma=2, weight=weight)
        loss_class = FocalLoss(gamma=2)
